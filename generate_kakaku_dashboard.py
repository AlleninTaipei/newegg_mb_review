"""
Kakaku Dashboard Generator
Reads kakaku_reviews.json and produces a self-contained kakaku_dashboard.html
Prices are in JPY; value score = rating / price × 10,000
"""
...
INPUT_FILE  = "kakaku_reviews.json"
OUTPUT_FILE = "kakaku_dashboard.html"
...
def build_html(products: list[dict], meta: dict) -> str:
    ...
    max_price  = max((p["price"] for p in products), default=100000)
    price_ceil = int((max_price // 10000 + 1) * 10000)
...
<head>
  ...
  <style>
    ...
    *  {{ box-sizing:border-box; margin:0; padding:0; }}
    ...
    .range-val {{ font-size:.78rem; color:var(--yellow); min-width:80px; }}
    .filters button {{
...
<header>
  ...
  <div class="header-text">
    ...
    <div class="sub">
      {source} ratings &amp; pricing &bull; Data as of {retrieved_date} &bull; Source: {source}.com &bull; Prices in JPY
    </div>
  </div>
</header>
...
<div class="filters">
  ...
  <input type="range" id="fPrice" min="1000" max="{price_ceil}" step="1000" value="{price_ceil}" />
...
<div class="picks-row">
  <div class="pick-card">
    <h3>Top Rated (min 5 reviews)</h3>
    <div id="topPicks"></div>
  </div>
  <div class="pick-card">
    <h3>Best Value (rating ÷ price × 10,000)</h3>
...
function valueScore(p) {{
  return p.price > 0 ? ((p.rating / p.price) * 10000).toFixed(2) : '0.00';
}}
function stars(r) {{
...
function renderPicks(full) {{
  const q5 = full.filter(p=>p.reviews>=5);

  const top5 = [...q5].sort((a,b)=>b.rating-a.rating).slice(0,5);
  ...
  const val5 = [...q5].sort((a,b)=>+valueScore(b)-+valueScore(a)).slice(0,5);
  ...
  document.getElementById('popularPicks').innerHTML = pop5.map(p=>
    `<div class="pick-item">
      ...
      <div class="pick-meta">
        ...
        <div class="pick-price">${{p.rating}}★ · ${{fmtPrice(p.price)}}</div>
      </div>
    </div>`).join('') || '<div class="empty">Not enough data</div>';
}}
...
def main():
    src = Path(INPUT_FILE)
    if not src.exists():
        print(f"ERROR: {INPUT_FILE} not found. Run kakaku_scraper.py first.")
        sys.exit(1)
