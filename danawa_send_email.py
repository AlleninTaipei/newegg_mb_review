...
Configuration — set these environment variables (or create a .env file):
  ...
  EMAIL_RECIPIENT_DANAWA    comma-separated list of recipients
...
def compute_price_diff() -> list:
    ...
    for name, ni in cur_map.items():
        ...
        elif oi.get("price_krw") != np:
            diffs.append((name, oi["price_krw"], np, np - oi["price_krw"]))
    return diffs
...
def send(subject: str | None = None) -> None:
    missing = [v for v in ("EMAIL_SENDER", "EMAIL_PASSWORD", "EMAIL_RECIPIENT_DANAWA") if not os.environ.get(v)]
    if missing:
